  ...
  useFactory: (configService: ConfigService) => {
    ...
    return {
      ...
      url: configService.get('DATABASE_URL') || 
        `postgresql://${configService.get('DB_USER', 'postgres')}:${configService.get('DB_PASSWORD', 'mejo2127')}@${configService.get('DB_HOST', 'localhost')}:${configService.get('DB_PORT', '5432')}/${configService.get('DB_NAME', 'auth_service_dev')}`,
